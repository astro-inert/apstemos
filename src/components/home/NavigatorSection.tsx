    ...
    <Section className="border-t border-border">
      ...
      <div className="mt-14 overflow-hidden rounded-2xl border border-border bg-card shadow-card">
        ...
        <div className="grid lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <ul className="border-b border-border lg:border-b-0 lg:border-r">{nav.subtopics.map((s,i)=><li key={s}><button onMouseEnter={()=>setActive(i)} onFocus={()=>setActive(i)} onClick={()=>setActive(i)} className={`flex min-h-[2.75rem] w-full items-center justify-between gap-3 border-b border-border px-5 py-2.5 text-left text-[14px] transition-colors last:border-b-0 sm:min-h-12 sm:px-6 sm:py-3.5 ${active===i?"border-l-2 border-primary bg-primary/5 text-primary":"hover:bg-elevated/60"}`}><span className="min-w-0 truncate font-medium">{s}</span><ChevronRight className={`h-4 w-4 shrink-0 transition-transform ${active===i?"translate-x-0.5 text-primary":"text-subtle"}`} /></button></li>)}</ul>
          <div className="p-6 sm:p-8"><MicroLabel>How this subtopic appears on the AP exam</MicroLabel><AnimatePresence mode="wait"><motion.div key={active} initial={reduced?{opacity:1}:{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={reduced?{opacity:1}:{opacity:0,y:-6}} transition={{type:"spring",stiffness:200,damping:24}} className="mt-6 grid gap-6 sm:grid-cols-2 sm:gap-8">{[{k:"MCQ",steps:activeGuidance.mcq},{k:"FRQ",steps:activeGuidance.frq}].map((col)=><div key={col.k}><div className="num text-[11px] font-semibold tracking-[0.18em] text-primary">{col.k}</div><ol className="mt-4 space-y-3">{col.steps.map((s,i)=><li key={s} className="flex gap-3 text-[13px] leading-relaxed"><span className="num shrink-0 text-[11px] text-subtle">{i+1}.</span><span className="text-secondary-foreground">{s}</span></li>)}</ol></div>)}</motion.div></AnimatePresence><Reveal delay={0.1}><p className="num mt-8 text-[10px] text-subtle">{activeSubtopic}</p></Reveal></div>
        </div>
      </div>
    </Section>
  );
