      ...
      <div className="mt-14 grid min-w-0 max-w-full grid-cols-[minmax(0,1fr)] overflow-hidden rounded-2xl border border-border bg-card shadow-card lg:grid-cols-2">
        <div className="min-w-0 max-w-full p-6 sm:p-8 lg:border-r lg:border-border">
          ...
          <div className="mt-7 space-y-2.5">
            {q.choices.map((c, i) => {
              ...
              return (
                <button key={i} onClick={() => setPicked(i)} className={`flex min-h-12 w-full items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm transition-all ${state}`}>
                  <span className="num text-[11px] text-subtle">{String.fromCharCode(65 + i)}</span>
                  <span className="min-w-0 flex-1"><LaTeX>{c}</LaTeX></span>
                  {revealed && isCorrect ? <Check className="h-4 w-4 shrink-0 text-primary" /> : null}
                  {revealed && isPicked && !isCorrect ? <X className="h-4 w-4 shrink-0 text-destructive" /> : null}
                </button>
              );
            })}
          </div>
          {revealed ? (
            <button onClick={() => setPicked(null)} className="num mt-6 text-[11px] uppercase tracking-[0.16em] text-subtle transition-colors hover:text-foreground">Reset question</button>
          ) : (
        ...
        <div className="relative min-w-0 max-w-full overflow-hidden border-t border-border bg-elevated/35 p-6 sm:min-h-[22rem] sm:p-8 lg:border-l-0 lg:border-t-0">
          <AnimatePresence mode="wait">
            {!revealed ? (
              <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative grid h-full min-h-[11rem] place-items-center sm:min-h-[18rem]">
                <p className="num max-w-[16rem] text-center text-[11px] uppercase leading-relaxed tracking-[0.16em] text-subtle">Diagnosis appears here the instant you answer</p>
              </motion.div>
            ) : (
              <motion.div key="diagnosis" initial={reduced ? { opacity: 1 } : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ type: "spring", stiffness: 160, damping: 20 }} className="relative">
                <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[12px] font-semibold ${picked === q.correct ? "border-primary/40 bg-accent/60 text-primary" : "border-destructive/40 bg-destructive/5 text-destructive"}`}>
                ...
                <dl className="mt-7 divide-y divide-border">
                  {[
                    ...
                    { k: "Mistake", v: picked === q.correct ? "None on this question" : q.mistake },
                    { k: "Pattern", v: picked === q.correct ? "No repeat pattern" : q.pattern },
                    { k: "Next", v: q.next },
                  ].map((row, i) => (
                    <motion.div key={row.k} initial={reduced ? { opacity: 1 } : { opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", stiffness: 200, damping: 24, delay: 0.06 * i }} className="flex items-baseline justify-between gap-4 py-3.5">
                ...
                {isSubjectLive(subject.id) ? (
                  <Link to="/practice" className="mt-7 inline-flex min-h-11 items-center gap-2 rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground shadow-card transition-colors hover:bg-primary/90">
                    Practice this weakness <ArrowRight className="h-4 w-4" />
                  </Link>
                ) : <div className="mt-7"><ComingSoon /></div>}
              </motion.div>
            )}
          </AnimatePresence>
        </div>
